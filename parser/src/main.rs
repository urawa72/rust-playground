...
use std::{fmt, io, iter::Peekable, str::FromStr};

// location from .0 to .1 range
...
fn compute_byte(input: &[u8], pos: usize, b: u8) -> Result<(u8, usize), LexError> {
    // pos is greater than input size
    if input.len() <= pos {
...
fn lex_plus(input: &[u8], start: usize) -> Result<(Token, usize), LexError> {
    compute_byte(input, start, b'+').map(|(_, end)| (Token::plus(Loc(start, end)), end))
}

fn lex_minus(input: &[u8], start: usize) -> Result<(Token, usize), LexError> {
    compute_byte(input, start, b'-').map(|(_, end)| (Token::minus(Loc(start, end)), end))
}

fn lex_asterisk(input: &[u8], start: usize) -> Result<(Token, usize), LexError> {
    compute_byte(input, start, b'*').map(|(_, end)| (Token::asterisk(Loc(start, end)), end))
}

fn lex_slash(input: &[u8], start: usize) -> Result<(Token, usize), LexError> {
    compute_byte(input, start, b'/').map(|(_, end)| (Token::slash(Loc(start, end)), end))
}

fn lex_lparen(input: &[u8], start: usize) -> Result<(Token, usize), LexError> {
    compute_byte(input, start, b'(').map(|(_, end)| (Token::lparen(Loc(start, end)), end))
}

fn lex_rparen(input: &[u8], start: usize) -> Result<(Token, usize), LexError> {
    compute_byte(input, start, b')').map(|(_, end)| (Token::rparen(Loc(start, end)), end))
}
...
fn lex(input: &str) -> Result<Vec<Token>, LexError> {
    ...
    let mut pos = 0;
    // update pos macro after sublexer
